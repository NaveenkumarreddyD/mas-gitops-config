...
{{IF_FALSE MANAGE_AUTO_GENERATE_ENCRYPTION_KEYS}}
    global_secrets:
      MXE_SECURITY_CRYPTO_KEY: "<path:${ACCOUNT_ID}/${CLUSTER_ID}/${INSTANCE_ID}/manage-crypto#cryptoKey>"
      MXE_SECURITY_CRYPTOX_KEY: "<path:${ACCOUNT_ID}/${CLUSTER_ID}/${INSTANCE_ID}/manage-crypto#cryptoxKey>"
{{END_IF}}
    mas_appws_spec:
      bindings:
        jdbc: system
...
{{IF_IN MANAGE_ATTACHMENT_PROVIDER s3-migration,s3}}
          importedCerts:
            - alias: powerscale-s3-subca
              crt: |
                <path:${ACCOUNT_ID}/${CLUSTER_ID}/${INSTANCE_ID}/manage-cos#powerscale_s3_subca>
            - alias: powerscale-s3-rootca
              crt: |
                <path:${ACCOUNT_ID}/${CLUSTER_ID}/${INSTANCE_ID}/manage-cos#powerscale_s3_rootca>
{{END_IF}}
          persistentVolumes:
